...
def main():
    ...
    #override settings given by argparse
    #with optuna trial.suggest_*
...
def objective(trial):
    ...
    # change dict in args into optuna trial values
    lr = trial.suggest_loguniform("lr", 5e-5, 5e-2)
    args.__dict__["lr"] = lr
    
    train_and_test(args,logger=logger, device=device,train_transform=train_transform,
            test_transform = test_transform, n_classes = n_classes, writer=writer)
...
def train_and_test(args, **kwargs):
    ...
    for trans in args.transforms:
        tr_names += "_" + trans
    save_path = f"{args.dataset}/{args.mode}_{args.mem_manage}_{args.stream_env}_msz{args.memory_size}_rnd{args.rnd_seed}{tr_names}"
    # MODIFIED : to fix np.save error
    os.makedirs(f"results/{args.dataset}", exist_ok=True)
    with open(f"results/{save_path}.npy", 'wb') as f:
        np.save(f, task_records["task_acc"])
